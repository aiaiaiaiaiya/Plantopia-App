...
try:
	while 1:
		...
		sql = 'SELECT pumpSpeed FROM user_control WHERE user_control.timestamp = (SELECT MAX(user_control.timestamp) FROM user_control WHERE pumpSpeed IS NOT NULL AND userID = '+userID+')'
		cur.execute(sql)
		db.commit()
