...
try:
	while 1:
		...
		sql = 'SELECT nutrientValve FROM user_control WHERE user_control.timestamp = (SELECT MAX(user_control.timestamp) FROM user_control WHERE nutrientValve IS NOT NULL AND userID = '+userID+')'
		cur.execute(sql)
		db.commit()
		...
		if nutrientValve[0] == 1:
			...
			cur = db.cursor()
			# Use all the SQL you like
			sql = 'INSERT INTO user_control VALUES ('+userID+', NULL, NULL, NULL, 0)'
			cur.execute(sql)
			db.commit()
			# print all the first cell of all the rows
